    ...
    return (<div className={style.tasks}>
        <label>
            <div className={style.checkbox}>
                <input className={style.fakeButton} type="checkbox" onChange={() => props.check(props.id)}
                       defaultChecked={props.isCheck}/>
                <img src={check} alt="Check" className={props.isCheck ? style.checkSvg : style.checkSvg && style.checkDone}/>
            </div>
        </label>
        <p className={props.isCheck ? style.taskText && style.doneText : style.taskText}>{props.text}</p>
        <label className={style.deleteContainer}><input
